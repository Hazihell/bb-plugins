...
- Nextra, actual shadcn Lyra components, Pierre Trees, Pierre Diffs, Bun, Oxlint, and a static Next.js export.
- A monospace stack for code and monospaced metadata, with no bundled font binary and no remote font request. A locally installed Berkeley Mono is used when present; every other machine gets the system monospace fallback.
